...
namespace SPAnamnese.Client.ServiceWeb
{
    public class PacientesSistemaServiceWeb
    {
        ...
        public async Task<PacienteSistemaDTO> GetByIdAsync(int id)
        {
            try
            {
                var response = await _httpClient.GetAsync($"api/PacientesSistema/GetPacienteById/{id}");
        ...
        {
            var response = await _httpClient.PostAsJsonAsync("api/PacientesSistema/CreatePaciente", pacienteDTO);
        ...
        public async Task<PacienteSistemaDTO> UpdatePacienteAsync(int id, PacienteSistemaFiltroDTO pacienteDTO)
        {
            try
            {
                var response = await _httpClient.PutAsJsonAsync($"api/PacientesSistema/UpdatePaciente/{id}", pacienteDTO);
                response.EnsureSuccessStatusCode();
                return await response.Content.ReadFromJsonAsync<PacienteSistemaDTO>() ?? new PacienteSistemaDTO();
            }
            catch
            ...
            int pageNumber = 1)
        {
            try
            {
                var url = $"api/PacientesSistema/PagedPacientes?pageSize={pageSize}&pageNumber={pageNumber}";
